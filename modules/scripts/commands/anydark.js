...
module.exports.config = {
  ...
  description: "Generate an AI image (anydark)",
  adminOnly: false,
  usePrefix: true,
  cooldown: 5,
};
...
module.exports.run = async function ({ event, args }) {
  if (args.length < 1) {
    return api.sendMessage(
      "Usage: anydark <prompt>\nExample: anydark beautiful girl",
      event.sender.id
    );
  }
  ...
  try {
    const response = await axios.get(`https://api.kenliejugarap.com/any-dark/?width=1024&height=1024&prompt=${encodeURIComponent(prompt)}`);
